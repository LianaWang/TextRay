...
from ..bbox.transforms import bbox2fori


def fourier_anchor_target(anchor_list,
                          valid_flag_list,
                          bbox_preds,
                          gt_bboxes_list,
                          gt_cheby_list,
...
def fourier_target_single(flat_anchors,
                          valid_flags,
                          bbox_pred,
                          gt_bboxes,
                          gt_cheby,
    ...
    bbox_targets = torch.zeros((num_valid_anchors, num_coefs), dtype=torch.float).cuda()
    # todo(chenyifeng): support other sampling points here?
    bbox_weights = torch.zeros((num_valid_anchors, num_coefs), dtype=torch.float).cuda()
    ...
    if len(pos_inds) > 0:
        deltas, weights = bbox2fori(sampling_result.pos_bboxes,
                                    bbox_pred[pos_inds, :],
                                    sampling_result.pos_gt_cheby,
                                    sampling_result.pos_gt_skeleton,
                                    target_means, target_stds, num_coords)
        bbox_targets[pos_inds, :] = deltas[:, :num_coefs]
        bbox_weights[pos_inds, :] = weights.unsqueeze(1) if cfg.use_centerness else 1.0
        ctr_targets[pos_inds, :] = deltas[:, num_coefs:num_coords]
    ...
    return (labels, label_weights, bbox_targets, bbox_weights, ctr_targets, ctr_weights, pos_inds, neg_inds)
